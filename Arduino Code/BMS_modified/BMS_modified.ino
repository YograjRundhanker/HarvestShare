...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    html += "<p><input type='submit' name='Display0' value='Display0'></p>";
    html += "<p><input type='submit' name='Charging' value='Charging'></p>";
...
void loop() {
  ...
  if (Charging) {
    ...
    while ((endtime - starttime) <=90000) // do this loop for up to 1000mS
    {
      percentage = starttime / 900;
      voltage = percentage*1618.3;
      lcd.begin();
      lcd.backlight();
      lcd.setCursor(0, 0);
      lcd.print("Energy: ");
